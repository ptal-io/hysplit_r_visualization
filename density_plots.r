...
pdf(paste("plots/",title,"_max_sp_density.pdf",sep=""), width=10, height=7)
par(mar=c(6,6,3,3))
#density(asdf, breaks=40, col='lightblue', main=paste(title," SH",sep=""), xlab="Specific Humdity")
plot(density(mmax, bw=0.3), type='n', main=paste(title," SH MAX",sep=""), ylim=range(0,0.35), xlim=range(0,20))
grid()
polygon(density(mmax, bw=0.3), col='lightblue')
dev.off()
